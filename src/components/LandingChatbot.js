  ...
  const handleSend = async (customQuery) => {
    ...
    try {
      ...
      if (lower.includes('price') || lower.includes('cost') || lower.includes('rupee') || lower.includes('plan')) {
        ...
        reply = `**SERVIQ Kitchen Display System (KDS)** replaces paper tickets completely! Orders appear instantly with color-coded SLA timers. Chefs can filter by station (Grill, Bar, Fryer, Bakery) and tap to notify floor staff when plated.`;
      } else if (lower.includes('demo') || lower.includes('trial') || lower.includes('book')) {
        reply = `We'd love to show you SERVIQ in action! Click the **'Schedule Demo'** button below or choose a preferred date/time on our demo booking page.`;
      } else {
        reply = `Thank you for asking! SERVIQ provides an autonomous operating system for modern restaurants in India — uniting QR digital ordering, multi-station KDS, recipe inventory depletion, and UPI billing into a single cloud console. Would you like to schedule a 15-minute live demo?`;
      }
    ...
    <div className={styles.floatingContainer}>
      ...
      {!isOpen && (
        <button
          ...
          aria-label="Open SERVIQ AI Concierge"
        >
          <div className={styles.pulseRing} />
          <Bot style={{ width: 17, height: 17 }} />
          <span>SERVIQ AI Concierge</span>
        ...
        {isOpen && (
          <div className={styles.chatWindow}>
            ...
            >
              <input
                type="text"
                placeholder="Ask anything about SERVIQ..."
                value={query}
